  ...
  return (
    <div className="min-h-screen pb-20" style={{background:"#f8fafc",fontFamily:"system-ui"}}>
      <div className="px-5 pt-14">
        <h1 className="text-xl font-light tracking-widest mb-1" style={{color:"#0ea5e9",fontFamily:"system-ui"}}>Learn</h1>
        <p className="text-xs mb-5" style={{color:"rgba(0,0,0,0.4)"}}>Evidence-based education</p>
        <div className="rounded-2xl p-4 mb-5" style={{background:"rgba(0,0,0,0.08)",border:"1px solid #0ea5e940"}}>
          <p className="text-[10px] font-semibold tracking-wider mb-1" style={{color:"#0ea5e9"}}>DAILY FACT</p>
          <p className="text-sm" style={{color:"#0f172a"}}>The average cycle is 28 days, but 21-35 days is normal.</p>
        </div>
        <div className="flex gap-2 overflow-x-auto mb-5 pb-1">{cats.map(c=>(<button key={c} onClick={()=>setActiveCat(c.toLowerCase())} className="px-4 py-2 rounded-full text-[11px] font-medium whitespace-nowrap border-0 cursor-pointer" style={{background:activeCat===c.toLowerCase()?"#0ea5e9":"rgba(0,0,0,0.04)",color:activeCat===c.toLowerCase()?"#f8fafc":"rgba(0,0,0,0.4)"}}>{c}</button>))}</div>
        {filtered.map((a,i)=>(<div key={i} className="rounded-2xl p-4 mb-3 cursor-pointer" style={{background:"rgba(0,0,0,0.04)"}}>
          <div className="flex justify-between mb-1"><span className="text-[10px] font-semibold" style={{color:"#0ea5e9"}}>{a.cat}</span><span className="text-[10px]" style={{color:"rgba(0,0,0,0.4)"}}>{a.time}</span></div>
