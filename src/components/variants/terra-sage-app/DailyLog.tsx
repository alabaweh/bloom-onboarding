  ...
  return (
    <div className="min-h-screen pb-20" style={{background:"#f4f1ec",fontFamily:"system-ui"}}>
      <div className="px-5 pt-14">
        <h1 className="text-xl font-light tracking-widest text-center mb-1" style={{color:"#6b8f5e",fontFamily:"Georgia, serif"}}>Daily Log</h1>
        ...
        <div className="space-y-3">{["Mood","Energy","Sleep","Cramps","Flow"].map(cat=>(<div key={cat} className="rounded-2xl p-4" style={{background:"rgba(107,143,94,0.08)",border:"1px solid rgba(0,0,0,0.06)"}}><p className="text-sm font-medium mb-2" style={{color:"#2d3a28"}}>{cat}</p><div className="flex gap-2">{[1,2,3,4,5].map(n=>(<button key={n} className="w-10 h-10 rounded-xl border-0 cursor-pointer text-xs" style={{background:n===3?"#6b8f5e":"transparent",color:n===3?"#ffffff":"rgba(45,58,40,0.5)",border:"1px solid rgba(0,0,0,0.06)"}}>{n}</button>))}</div></div>))}</div>
