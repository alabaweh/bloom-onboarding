  ...
  return (
    <div className="min-h-screen pb-20" style={{background:"#f5f0eb",fontFamily:"system-ui"}}>
      <div className="px-5 pt-14">
        <h1 className="text-xl font-light tracking-widest mb-1" style={{color:"#c07a50",fontFamily:"Georgia, serif"}}>Learn</h1>
        <p className="text-xs mb-5" style={{color:"rgba(0,0,0,0.4)"}}>Evidence-based education</p>
        <div className="rounded-2xl p-4 mb-5" style={{background:"rgba(0,0,0,0.08)",border:"1px solid #c07a5040"}}>
          <p className="text-[10px] font-semibold tracking-wider mb-1" style={{color:"#c07a50"}}>DAILY FACT</p>
          <p className="text-sm" style={{color:"#3c2f2f"}}>The average cycle is 28 days, but 21-35 days is normal.</p>
        </div>
        <div className="flex gap-2 overflow-x-auto mb-5 pb-1">{cats.map(c=>(<button key={c} onClick={()=>setActiveCat(c.toLowerCase())} className="px-4 py-2 rounded-full text-[11px] font-medium whitespace-nowrap border-0 cursor-pointer" style={{background:activeCat===c.toLowerCase()?"#c07a50":"rgba(0,0,0,0.04)",color:activeCat===c.toLowerCase()?"#f5f0eb":"rgba(0,0,0,0.4)"}}>{c}</button>))}</div>
        {filtered.map((a,i)=>(<div key={i} className="rounded-2xl p-4 mb-3 cursor-pointer" style={{background:"rgba(0,0,0,0.04)"}}>
          <div className="flex justify-between mb-1"><span className="text-[10px] font-semibold" style={{color:"#c07a50"}}>{a.cat}</span><span className="text-[10px]" style={{color:"rgba(0,0,0,0.4)"}}>{a.time}</span></div>
          <p className="text-sm font-medium" style={{color:"#3c2f2f"}}>{a.title}</p>
        </div>))}
      </div>
      <Nav active="insights"/>
    </div>
