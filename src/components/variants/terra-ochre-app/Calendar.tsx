  ...
  return (
    <div className="min-h-screen pb-20" style={{background:"#2a1f14",fontFamily:"system-ui"}}>
      <div className="px-5 pt-14">
        <h1 className="text-xl font-light tracking-widest text-center mb-1" style={{color:"#d4a24e",fontFamily:"Georgia, serif"}}>January 2025</h1>
        <div className="flex gap-1 p-1 rounded-full mb-6" style={{background:"rgba(212,162,78,0.08)"}}>{modes.map(m=>(<button key={m.key} onClick={()=>setMode(m.key)} className="flex-1 py-2 rounded-full text-[10px] font-medium border-0 cursor-pointer" style={{background:mode===m.key?"rgba(255,255,255,0.1)":"transparent",color:mode===m.key?"#d4a24e":"rgba(255,255,255,0.4)"}}>{m.label}</button>))}</div>
        <div className="grid grid-cols-7 gap-1 mb-2">{["M","T","W","T","F","S","S"].map((d,i)=>(<div key={i} className="text-center text-[10px] py-1" style={{color:"rgba(240,228,208,0.4)"}}>{d}</div>))}</div><div className="grid grid-cols-7 gap-1 mb-5">{Array.from({length:2}).map((_,i)=><div key={`e${i}`}/>)}{Array.from({length:31}).map((_,i)=>{const day=i+1;const isPeriod=[1,2,3,4,5].includes(day);const isFertile=[12,13,14,15,16].includes(day);return(<button key={day} className="aspect-square rounded-xl flex items-center justify-center border-0 cursor-pointer text-xs" style={{background:isPeriod?"rgba(232,121,160,0.2)":isFertile?"rgba(13,148,136,0.15)":"transparent",color:isPeriod?"#e879a0":isFertile?"#0d9488":"#f0e4d0"}}>{day}</button>);})}</div>
