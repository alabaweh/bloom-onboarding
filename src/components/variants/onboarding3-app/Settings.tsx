...
export default function Settings() {
  ...
  const Toggle=({on,onToggle}:{on:boolean;onToggle:()=>void})=>(<button onClick={onToggle} className="w-10 h-6 rounded-full relative border-0 cursor-pointer" style={{background:on?"#d4a060":"rgba(0,0,0,0.04)"}}><div className="w-4 h-4 rounded-full absolute top-1 transition-all" style={{background:on?"#fff8f0":"rgba(0,0,0,0.4)",left:on?22:4}}/></button>);
  return (
    <div className="min-h-screen pb-20" style={{background:"#fff8f0",fontFamily:"system-ui"}}>
      <div className="px-5 pt-14">
        <h1 className="text-xl font-light tracking-widest mb-6" style={{color:"#d4a060",fontFamily:"Georgia, serif"}}>Settings</h1>
        <p className="text-[10px] tracking-wider mb-3" style={{color:"rgba(0,0,0,0.4)"}}>NOTIFICATIONS</p>
        <div className="mb-5">{[{l:"Push Notifications",d:"Daily reminders",on:notifs,t:()=>setNotifs(!notifs)},{l:"Period Reminder",d:"3 days before",on:periodRemind,t:()=>setPeriodRemind(!periodRemind)}].map(s=>(<div key={s.l} className="flex items-center justify-between py-3" style={{borderBottom:"1px solid rgba(0,0,0,0.04)"}}><div><p className="text-sm font-medium" style={{color:"#3d2b1f"}}>{s.l}</p><p className="text-xs" style={{color:"rgba(0,0,0,0.4)"}}>{s.d}</p></div><Toggle on={s.on} onToggle={s.t}/></div>))}</div>
        <p className="text-[10px] tracking-wider mb-3" style={{color:"rgba(0,0,0,0.4)"}}>PRIVACY & SECURITY</p>
        <div className="mb-5">
          <div className="flex items-center justify-between py-3" style={{borderBottom:"1px solid rgba(0,0,0,0.04)"}}><div><p className="text-sm font-medium" style={{color:"#3d2b1f"}}>Biometric Lock</p><p className="text-xs" style={{color:"rgba(0,0,0,0.4)"}}>Face ID / Touch ID</p></div><Toggle on={bio} onToggle={()=>setBio(!bio)}/></div>
          <div className="flex items-center justify-between py-3" style={{borderBottom:"1px solid rgba(0,0,0,0.04)"}}><div><p className="text-sm font-medium" style={{color:"#3d2b1f"}}>Data Encryption</p><p className="text-xs" style={{color:"rgba(0,0,0,0.4)"}}>AES-256 enabled</p></div><svg width="14" height="14" viewBox="0 0 24 24" fill="#d4a060"><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg></div>
        </div>
        <p className="text-[10px] tracking-wider mb-3" style={{color:"rgba(0,0,0,0.4)"}}>DATA</p>
        <div className="mb-5">
          <button className="w-full py-3 text-sm font-medium text-left border-0 cursor-pointer bg-transparent" style={{color:"#3d2b1f",borderBottom:"1px solid rgba(0,0,0,0.04)"}}>Export My Data</button>
          <button className="w-full py-3 text-sm font-medium text-left border-0 cursor-pointer bg-transparent" style={{color:"#c06840",borderBottom:"1px solid rgba(0,0,0,0.04)"}}>Delete Account</button>
        </div>
        <p className="text-[10px] tracking-wider mb-3" style={{color:"rgba(0,0,0,0.4)"}}>ABOUT</p>
        <div className="flex justify-between py-3" style={{borderBottom:"1px solid rgba(0,0,0,0.04)"}}><span className="text-sm" style={{color:"#3d2b1f"}}>Version</span><span className="text-sm" style={{color:"rgba(0,0,0,0.4)"}}>2.1.0</span></div>
        <div className="flex justify-between py-3" style={{borderBottom:"1px solid rgba(0,0,0,0.04)"}}><span className="text-sm" style={{color:"#3d2b1f"}}>Privacy Policy</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(0,0,0,0.4)" strokeWidth="1.5"><path d="M9 5l7 7-7 7"/></svg></div>
